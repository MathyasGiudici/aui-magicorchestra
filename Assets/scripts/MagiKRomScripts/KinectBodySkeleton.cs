...
[Serializable]
    ...
    public bool isRightHandClosed()
    {
        ...
        List<string> gestures2 = new List<string>();
        gestures2.AddRange(GestureMask.Split('|'));
        foreach (string s in gestures2)
        {
            gestures.Add(s.Split(':').First());
        }
        foreach (string s in gestures)
        {
            return s.Contains("CLOSEHANDRIGHT");
        }
        return false;
    }
    ...
    {
        List<string> gestures = new List<string>();
        gestures.AddRange(GestureMask.Split('|'));
        foreach (string s in gestures)
        {
            return s.Contains("CLOSEHANDLEFT");
        }
        return false;
    }
}
