...
public class StreamingAssetManager : MonoBehaviour {
    ...
    public void loadAudioClipFromStreamingAsset(string folder, string filename, AudioSource destination) {
        ...
        if(notfound) {
            directoryInfo = new DirectoryInfo("C://LUDOMI/Resources/sounds");
    ...
    public void loadImageFromStreamingAsset(string folder, string filename, Image destination)
    {
        ...
        if (notfound)
        {
            directoryInfo = new DirectoryInfo("C://LUDOMI/Resources/images");
            allFiles = directoryInfo.GetFiles("*.*");
            foreach (FileInfo file in allFiles)
            {
                if (file.Name == filename)
    ...
    {
        Image i = _DestinationImage.ElementAt(0);
        ...
        else
        {
            ...
            yield return www;
            i.sprite = Sprite.Create(www.texture, new Rect(0, 0, www.texture.width, www.texture.height), new Vector2(0.5f, 0.5f));
            i.sprite.name = playerFile.Name;
            
        }
    }
    IEnumerator LoadSpriteFromAsset(FileInfo playerFile)
    {
        SpriteRenderer s = _DestinationSprite.ElementAt(0);
        ...
        else
        {
            ...
            WWW www = new WWW(wwwPlayerFilePath);
            yield return www;
            s.sprite = Sprite.Create(www.texture, new Rect(0, 0, www.texture.width, www.texture.height), new Vector2(0.5f, 0.5f));
            s.sprite.name = playerFile.Name;
